...
The [`Embedding`](https://www.tensorflow.org/api_docs/python/tf/keras/layers/Embedding) layer creates a vocabulary space, giving each word its own spot in that space. Furthermore, the spots of the words are set in a way that the distance and direction between words would indicate relatedness and relationships.We'll give this layer a name to ease the process of examining it later.
